...
# Pirobola: danno INT + bruciatura (INT/3 per turno, 3 turni). 8 MP. Range INT/3.
...
func use_targeted(tile: Vector2i) -> void:
	...
	var burn_dmg: int = maxi(1, int_v / 3)
	_deal_damage(_get_player(), enemy, int_v, false, "magic")
	var sem: Node = _runtime.get_node_or_null("/root/StatusEffectManager")
	if sem:
		sem.call("apply", enemy, {
			"id": "burn", "source": "piromante",
			"duration_turns": BURN_TURNS, "stacking": "refresh",
			"data": {"dmg_per_turn": burn_dmg}
		})
	_combat_log("Pirobola! %d danni + bruciatura (%d/t × %d t)." % [int_v, burn_dmg, BURN_TURNS])
